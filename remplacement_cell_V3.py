...
def selection_fut(type_produit,numero_serie,cursor):
        ...
        cursor.execute("SELECT bib.poids FROM bibliotheque AS bib JOIN cellules AS cell ON bib.reference_cellule = cell.reference_cellule WHERE cell.numero_serie_cellule = %s",(numero_serie,))
        poids = cursor.fetchall()[0][0]
        poids= float(poids)
        type_fut="Screlec"
        cursor.execute("SELECT id_fut from futs WHERE exutoire=%s and etat_fut=%s limit 1",(type_fut,"en cours"))
        fut=cursor.fetchall()
        ...
        if not fut : 
            ...
            cursor.execute("SELECT id_fut,poids from futs WHERE exutoire=%s and etat_fut=%s limit 1",(type_fut,"en cours"))
            data_fut=cursor.fetchall()
            fut,poids_fut=data_fut[0]
            poids_tot= poids_fut + poids 
            cursor.execute("UPDATE futs SET poids=%s where id_fut=%s",(poids_tot,fut))
...
def trouver_remplacement(cellule, df_disponibles,capa_origine,resi_origine):
    ...
    df_filtre['ecart_capacite'] = np.abs(df_filtre['capacite_decharge_cellule_mesuree'] - capa_origine)
    df_filtre['ecart_resistance'] = np.abs(df_filtre['resistance_interne_cellule_mesuree'] - resi_origine)
...
def maj_bdd_origine(conn, id_cellule,commentaire=None):
    ...
    try:
        cursor.execute(
            "UPDATE cellules SET disponibilite = %s, affectation_produit = NULL, commentaire = %s WHERE numero_serie_cellule = %s",
...
def maj_bdd_remplaçante(conn, id_cellule, produit, commentaire=None):
    ...
    try:
        cursor.execute(
            "UPDATE cellules SET disponibilite = %s, affectation_produit = %s, commentaire = %s WHERE numero_serie_cellule = %s",
...
def inserer_recyclage(conn, numero_serie):
    ...
    try:
        ...
        insert_query = "INSERT INTO recyclage_dechet (numero_serie,type_objet,sur_site,date_recyclage,fut) VALUES (%s,%s,%s,%s,%s)"
        cursor.execute(insert_query, (numero_serie,"Cellule",'oui',date_du_jour,num_fut))
        cursor.execute("UPDATE cellules SET disponibilite=%s, exutoire=%s WHERE numero_serie_cellule = %s", ("NON DISPO","recyclage",numero_serie))  
        conn.commit()
    except Error as e:
        print(f"Erreur lors de l'insertion dans la BDD: {e}")
    finally:
...
def main(fichier_excel, host, database, user, password, ref_cell,type_carac):
    ...
    try:
        query = "SELECT numero_serie_cellule, reference_cellule, capacite_decharge_cellule_mesuree, resistance_interne_cellule_mesuree, disponibilite FROM cellules WHERE disponibilite = 'DISPO' and reference_cellule = %s and type_carac= %s"
        df_disponibles = pd.read_sql(query, conn, params=[ref_cell,type_carac])
        
        file_path="C:/Users/User/Desktop/Plateaux actuel/plateaux tot.xlsx"
        ...
        for _, cellule in df_excel.iterrows():
            ...
            cursor=conn.cursor()
            cursor.execute("select capacite_decharge_cellule_mesuree,resistance_interne_cellule_mesuree from cellules where numero_serie_cellule=%s",(numero_serie,))
            ...
            remplacements.append({
                ...
                'capa_cellule_remplaçante': meilleure_cellule['capacite_decharge_cellule_mesuree'],
                'resistance_cellule_origine': resi_origine,
                'resistance_cellule_remplaçante': meilleure_cellule['resistance_interne_cellule_mesuree']
            })
        ...
        chemin_rapport = "C:/Users/User/Desktop/Remplacement NA 1 cell.xlsx"
...
fichier_excel = "C:/Users/User/Desktop/Remplacement NA/NA_1_cell.xlsx"
"""
host = "localhost"
